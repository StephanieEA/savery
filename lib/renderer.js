...
const showRecipe = (title, link) => {
  $('#recipe-box').append(`
    ...
    <button class="recipe-heading">${title}</button>
    <webview id="${title}" class="hide" src="${link}" style="display:inline-flex; width:640px; height:480px"></webview>
  `)
}
...
const renderRecipes = (recipes) => {
  JSON.parse(recipes).all_recipes.map(recipe => $('#recipe-box').append(`
    <h2><a href="${recipe.link}">${recipe.title}</a>
    </h2>
    <button class="recipe-heading">${recipe.title}</button>
